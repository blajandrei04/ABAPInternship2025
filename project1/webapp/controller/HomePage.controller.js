...
            if (oViewModel) {
                oViewModel.setProperty("/selectedFeedback", null);
...
    const sFbId = oSelectedItem.FB_ID;

    console.log("[DEBUG] Fetching full feedback details for FB_ID:", sFbId);

    // Fetch full feedback details before navigating
    const oODataModel = this.getOwnerComponent().getModel();
    oODataModel.read("/FB_CATSet('" + sFbId + "')", {
        success: function(oData) {
            console.log("[DEBUG] Successfully fetched full feedback details. Data received:", oData);
            
            // Map backend data properties to what the view expects
            const oFullFeedbackDetails = {
                ...oSelectedItem,
                FB_RATING: oData.CATEGORY_RATING,
                COMMENT: oData.CATEGORY_COMMENT
            };

            // Get the component's model and set the data
            const oViewModel = this.getOwnerComponent().getModel("view");
            oViewModel.setProperty("/selectedFeedback", oFullFeedbackDetails);
            oViewModel.setProperty("/fbVisible", true);
            console.log("[DEBUG] Navigating to FeedbackDetails page. Final object in model:", oFullFeedbackDetails);
            this.getRouter().navTo("RouteFeedbackDetails");
        }.bind(this),
        error: function(oError) {
            console.error("[DEBUG] Error fetching FB_CATSet for FB_ID:", sFbId, "Error:", oError);
            sap.m.MessageBox.error("Failed to load feedback details. Please try again.");
            const oViewModel = this.getOwnerComponent().getModel("view");
            oViewModel.setProperty("/selectedFeedback", null);
            oViewModel.setProperty("/fbVisible", false);
        }.bind(this)
